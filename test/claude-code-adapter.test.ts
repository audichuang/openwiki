...
describe("claudeCodeAdapter.buildArgs", () => {
  test("builds headless stream-json args with the appended system prompt", () => {
    expect(claudeCodeAdapter.buildArgs(baseSpec, null)).toEqual([
      "-p",
      "--output-format",
      "stream-json",
      "--verbose",
  ...
  test("omits --model for the subscription default and adds it otherwise", () => {
    expect(claudeCodeAdapter.buildArgs(baseSpec, null)).not.toContain(
      "--model",
    );

    const args = claudeCodeAdapter.buildArgs(
      { ...baseSpec, modelId: "opus" },
      null,
    );
...
describe("claudeCodeAdapter.createParser().parse", () => {
  test("system init yields a session event and a debug event", () => {
    const events = claudeCodeAdapter.createParser().parse({
      type: "system",
      subtype: "init",
      session_id: "sess-abc",
  ...
  test("tool results become tool_end events with error status mapping", () => {
    const ok = claudeCodeAdapter.createParser().parse({
      type: "user",
      message: {
        role: "user",
        content: [
    ...
    const failed = claudeCodeAdapter.createParser().parse({
      type: "user",
      message: {
        role: "user",
        content: [
  ...
  test("result events map success and error subtypes", () => {
    expect(
      claudeCodeAdapter.createParser().parse({
        type: "result",
        subtype: "success",
        is_error: false,
        result: "done",
  ...
  test("unknown lines are ignored", () => {
    expect(claudeCodeAdapter.createParser().parse("not json-shaped")).toEqual(
      [],
    );
    expect(claudeCodeAdapter.createParser().parse({ type: "mystery" })).toEqual(
      [],
    );
    expect(claudeCodeAdapter.createParser().parse(null)).toEqual([]);
  });
});
