...
export function providerRequiresApiKey(provider: OpenWikiProvider): boolean {
  return (
    ...
    getApiProviderConfig(provider).apiKeyEnvKey !== undefined
  );
}
...
export function getProviderProjectEnvKey(
  ...
  return getApiProviderConfig(provider).projectEnvKey;
}
...
export function getProviderLocationEnvKey(
  ...
  return getApiProviderConfig(provider).locationEnvKey;
}
 ...
 * Agent CLI providers never require OpenWiki-managed secrets.
 */
...
// Returns the built-in providers whose known model options include the given
// model ID by exact match, excluding the provider passed in. Used to warn when
// a saved model plainly belongs to a different provider (e.g. an Anthropic
// model left over while the provider is now OpenAI). Exact matching avoids
// false positives from namespaced overlaps such as OpenRouter's
// "anthropic/claude-...". Returns [] for custom/unknown ids, so gateway and
