...
export async function runAgentCli(
  ...
  emitDebug(
    options,
    `engine=${adapter.id} binary=${binary} version=${install.version ?? "unknown"} delivery=${adapter.promptDelivery}`,
  );

  const usePromptFile = adapter.promptDelivery === "prompt-file";
  ...
  if (usePromptFile) {
    ...
    const fileBody =
      spec.systemPrompt.trim().length > 0
        ? `${spec.systemPrompt}\n\n---\n\n${spec.prompt}`
        : spec.prompt;
    await writeFile(promptFilePath, fileBody, {
      encoding: "utf8",
      mode: 0o600,
    });
  }
  ...
  try {
    const child = spawn(binary, adapter.buildArgs(spec, promptFilePath), {
      cwd: spec.cwd,
      detached: true,
      stdio: [usePromptFile ? "ignore" : "pipe", "pipe", "pipe"],
    });
    ...
    if (!usePromptFile) {
      if (child.stdin === null) {
        throw new Error(
          `${providerConfig.label} stdin was not available for prompt delivery.`,
        );
      }

      child.stdin.on("error", () => {
        // EPIPE: child exited before consuming stdin. Failure reported via close.
      });
    ...
    if (child.stderr === null || child.stdout === null) {
      throw new Error(
        `${providerConfig.label} stdio pipes were not available after spawn.`,
      );
    }
